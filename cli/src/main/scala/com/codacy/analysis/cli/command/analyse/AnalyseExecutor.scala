...
import com.codacy.analysis.core.configuration.EngineConfiguration
...
class AnalyseExecutor(formatter: Formatter,
  ...
  private def getToolConfiguration(tool: Tool,
    ...
    val (baseSubDir, extraValues) = getExtraConfiguration(properties.enginesConfiguration, tool)
    (for {
  ...
  private def getExtraConfiguration(enginesConfiguration: Option[Map[String, EngineConfiguration]],
  ...
  def tools(toolInput: Option[String],
    ...
    def fromRemoteConfig: Either[CLIError, Set[Tool]] = {
      properties.toolConfigurations.left.map(CLIError.NoRemoteProjectConfiguration).flatMap { toolConfiguration =>
        val toolUuids = toolConfiguration.filter(_.isEnabled).map(_.uuid)
        toolCollector
